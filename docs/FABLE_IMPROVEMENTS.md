# Tabento — Fable Improvements Plan

Status: proposal / living document. Author model: **Fable 5**. Created: 2026-07-02.

This document plans a set of **major** features for Tabento that lean into what Fable is
best at: spatial, animated, design-forward interfaces and the systems work needed to make
them cohere. Every proposal here is grounded in the current codebase (vanilla JS, MV3, no
build step, local-first, no host permissions) and respects the visual signature laid out in
[`DESIGN.md`](../DESIGN.md).

> **The Fable angle.** Opus and Sonnet plan and build correctly. Fable's edge is *taste under
> constraint* — designing motion, spatial layout, and a coherent navigation feel inside a
> hand-written vanilla-JS app with zero framework, then keeping it consistent across 13 themes
> and light/dark. The features below are chosen because their value lives in execution quality:
> a timeline that *feels* alive, an explorer that *feels* like a filesystem, a calendar that
> reads at a glance. That is the work this plan optimizes for.
...
*Why Fable:* getting column-browser focus/scroll/selection to feel native in vanilla JS — the
"snap to reveal the next column," the retained selection per column, the keyboard model — is
...
*Why Fable:* the whole value is motion design and time-scale legibility — easing curves,
